...
def get_layer_inputs(model, test_input, skip=None, outs=None):
    ...
    for i in range(len(outs)):
        ...
        for input_index in range(len(test_input)):
            inputs_for_layer.append(np.add(np.dot(outs[i - 1][0][input_index] \
              if i > 0 else test_input[input_index], weights), biases))
...
def generate_adversarial(original_input, method, model, target=None, target_class=None, 
                         sess=None, **kwargs):
    if not hasattr(generate_adversarial, "attack_types"):
        generate_adversarial.attack_types = {'fgsm': FastGradientMethod, 
